(()=>{function e(N=!1){Array.from(document.body.querySelectorAll(":scope > :not(header)")).forEach(C=>C.inert=N)}document.addEventListener("click",function(N){let C;if(C=N.target.closest("[data-toggle-pressed]")){let K=C.getAttribute("aria-pressed")!=="true";C.setAttribute("aria-pressed",K.toString()),C.matches(".header-menu-toggle")&&e(K)}else C=N.target,C.closest(".header-themepicker")||document.querySelector(".header-themepicker-toggle").setAttribute("aria-pressed","false")});document.addEventListener("keyup",function(N){let C=document.querySelector('[data-toggle-pressed][aria-pressed="true"]');C&&(N.key==="Escape"||N.keyCode===27)&&(C.setAttribute("aria-pressed","false"),e(!1),C.focus())});window.matchMedia(`(min-width:${globalBreakpoint})`).addEventListener("change",function(N){Array.from(document.querySelectorAll(".header-menu-toggle, .header-themepicker-toggle")).forEach(C=>C.setAttribute("aria-pressed","false")),e(!1)});console.log(`%c
